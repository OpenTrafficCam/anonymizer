...
def main(
    ...
    weights_path=str(pathlib.Path(__file__).parents[2] / "weights"),
):
    ...
    detectors = {
        "face": Detector(
            kind="face", weights_path=get_weights_path(weights_path, kind="face")
        ),
        "plate": Detector(
            kind="plate", weights_path=get_weights_path(weights_path, kind="plate")
        ),
    }
